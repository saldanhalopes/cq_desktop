...
public class ColunaDAO {
    ...
    public void createColuna(Coluna col) {
        ...
        try {
            stmt = conn.prepareStatement("INSERT INTO tb_coluna "
                    ...
                    + "data_performance, user_name_performance) "
                    + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public List<Coluna> readColuna(String _setor, int status) {
        ...
        try {
            ...
            stmt = conn.prepareStatement("SELECT tb_coluna.*, "
                    ...
                    + "WHERE 1 = 1 " + where
                    + "ORDER BY tb_coluna.coluna_id DESC LIMIT 10000");
            ...
            while (rs.next()) {
                ...
                user.setUser(rs.getString("user_name_ativacao"));
                col.setUser_ativacao(user);
                user.setUser(rs.getString("user_name_descarte"));
                col.setUser_descarte(user);
                user.setUser(rs.getString("user_name_performance"));
    ...
    public void updateColuna(Coluna col) {
        ...
        try {
            ...
            stmt.setInt(19, col.getColuna_id());
            stmt.executeUpdate();
    ...
    public void selectUltimaColuna(Coluna col) {
        ...
        ResultSet rs = null;
        try {
            stmt = conn.prepareStatement("SELECT tb_coluna.cod_coluna "
                    + "FROM tb_coluna WHERE tb_coluna.tipo_coluna = ? "
                    + "ORDER BY tb_coluna.cod_coluna DESC LIMIT 1");
    ...
    public static void printColuna(String user, String setor, int status) {
        ...
        String stmt = "SELECT tb_coluna.*, tb_metodologia.cod_metodo, "
                ...
                + "ON tb_coluna.setor_id = tb_setor.setor_id "
                + "WHERE 1 = 1 " + where
                + " ORDER BY tb_setor.sigla_setor, tb_coluna.cod_coluna ASC, tb_coluna.vaga_coluna_id ASC ";
    ...
    public void createLogColuna(Coluna col) {
        ...
        try {
            stmt = conn.prepareStatement("INSERT INTO tb_log_coluna "
                    + "(coluna_id, system_name, user_name, data_registro, "
                    + "sentido, precoluna, prefiltro) "
                    + "VALUES (?,?,?,?,?,?,?)");
            stmt.setInt(1, col.getColuna_id());
            stmt.setString(2, col.getCromatografo().getSystem_name());
            stmt.setString(3, col.getUser_log_coluna().getUser());
            stmt.setString(4, col.getData_registro_log());
            stmt.setInt(5, (col.isSentido()) ? 1 : 0);
            stmt.setInt(6, (col.isPrefiltro()) ? 1 : 0);
            stmt.setInt(7, (col.isPrecoluna()) ? 1 : 0);
            stmt.executeUpdate();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Erro: " + ex);
        } finally {
    ...
    public List<Coluna> readColunaBySystem(String system_name) {
        ...
        List<Coluna> colunas = new ArrayList<>();
        try {
            stmt = conn.prepareStatement("SELECT tb_log_coluna.log_coluna_id, tb_coluna.*, "
                    ...
                    + "ORDER BY tb_log_coluna.log_coluna_id DESC LIMIT 20 ");
            ...
            while (rs.next()) {
                ...
                col.setVaga_coluna(rs.getInt("vaga_coluna_id"));
                col.setObs_coluna(rs.getString("obs_coluna"));
                col.setData_registro_log(rs.getString("data_registro"));
    ...
    public void selectUltimaColunaBySystem(Coluna col) {
        ...
        try {
            stmt = conn.prepareStatement("SELECT tb_log_coluna.log_coluna_id, tb_coluna.*, "
                    ...
                    + "ON tb_coluna.setor_id = tb_setor.setor_id  "
                    + "WHERE tb_log_coluna.system_name = ?  "
                    + "ORDER BY tb_log_coluna.log_coluna_id DESC LIMIT 1 ");
